...
class TestIngestion:
    def test_load_pdfs_missing_folder(self, tmp_path):
        from app.core.ingestion.loader import load_pdfs_from_folder
...
class TestChunking:
    def test_clean_text_empty(self):
        from app.core.chunking.chunker import clean_text
        assert clean_text("") == ""
    ...
    def test_build_parent_child_chunks_empty(self, tmp_path):
        from app.core.chunking.chunker import build_parent_child_chunks
...
class TestEmbeddings:
    def test_constants(self):
        ...
        from app.core.embeddings.embedder import E5_EMBEDDING_DIM, E5_EMBEDDING_MODEL_NAME
        assert E5_EMBEDDING_DIM == get_config().model.embedding.dim
        assert "e5" in E5_EMBEDDING_MODEL_NAME.lower()

    def test_config_defaults(self):
        from app.core.app_config import get_config
        from app.core.embeddings.embedder import E5EmbeddingConfig
...
class TestHelpers:
    def test_clean_text(self):
        from app.core.utils.helpers import clean_text
        assert clean_text("  test  ") == "test"
        assert clean_text("") == ""

    def test_build_query_structure(self):
        from app.core.utils.helpers import build_query
    ...
    def test_build_source_label_with_page(self):
        from app.core.utils.helpers import build_source_label
    ...
    def test_format_context_items(self):
        from app.core.utils.helpers import format_context_items
    ...
    def test_build_source_payload_index(self):
        from app.core.utils.helpers import build_source_payload
    ...
    def test_split_blocks(self):
        from app.core.utils.helpers import split_blocks
        text = "block một\n\nblock hai\n\nblock ba"
        blocks = split_blocks(text)
        assert len(blocks) == 3

    def test_coerce_text_string(self):
        from app.core.utils.helpers import coerce_text
...
class TestLLMClient:
    def test_constants(self):
        from app.core.llm.llm_client import DEFAULT_LLM_MODEL, RETRY_DELAYS, RETRYABLE_STATUS_CODES
        assert DEFAULT_LLM_MODEL
        assert len(RETRY_DELAYS) == 4
        assert 429 in RETRYABLE_STATUS_CODES

    def test_normalize_base_url_strips_trailing(self):
        from app.core.llm.llm_client import _normalize_base_url
        url = _normalize_base_url("https://api.example.com/v1/")
        assert not url.endswith("/")

    def test_is_shopaikey(self):
        from app.core.llm.llm_client import _is_shopaikey
    ...
    def test_resolve_model_name_returns_string(self):
        from app.core.llm.llm_client import resolve_model_name
...
class TestPrompts:
    def test_historian_prompt_has_placeholders(self):
        from app.core.prompts.prompt_templates import HISTORIAN_PROMPT
    ...
    def test_persona_prompt_has_placeholders(self):
        from app.core.prompts.prompt_templates import PERSONA_PROMPT
...
class TestVectorStore:
    def test_parent_id_to_uuid_stable(self):
        from app.core.vectordb.vector_store import parent_id_to_uuid
    ...
    def test_collection_names(self):
        from app.core.vectordb.vector_store import COLLECTION_NAME, PARENT_COLLECTION_NAME
