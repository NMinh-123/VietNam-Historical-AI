...
def _is_retryable_gemini_exception(exc: Exception) -> bool:
    ...
    Lỗi cấu hình (model_not_found, invalid_model) không retry vì
    retry chỉ làm chậm fail mà không giải quyết được vấn đề.
    """
    error_text = str(exc).lower()

    non_retryable_markers = ("model_not_found", "invalid_model", "model not found")
