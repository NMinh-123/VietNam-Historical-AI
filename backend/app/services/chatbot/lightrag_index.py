...
from services.chatbot.index.ingest_support import (
...
from services.chatbot.index.runtime import EmbeddingFunc, LightRAG
...
async def ingest_to_lightrag(
    ...
    """
    # Tầng 1: dedup trong cùng một lần chạy theo content hash
    ...
    try:
        # Drain queue cũ trước để tránh trộn batch mới với trạng thái dang dở
        await _resume_existing_pipeline_if_needed(
            rag,
            resume_existing_queue=resume_existing_queue,
        )

        # Tầng 2: dedup theo doc_id đã tồn tại trong LightRAG storage
        ...
        for batch_index, batch_records in enumerate(batches, start=1):
            ...
            try:
                ...
                # Ghi nhận lỗi vào manifest để có thể audit hoặc resume sau
            ...
            # Cập nhật trạng thái thực tế từ LightRAG sau khi batch hoàn tất
        ...
        # Đảm bảo giải phóng storage dù thành công hay lỗi
