...
def _prepare_qdrant_collection(
    ...
    """Tạo lại collection với schema dense+sparse đúng cho hybrid search."""
...
def _ensure_qdrant_collection(
    ...
    """Tạo collection nếu chưa có, hoặc tái sử dụng collection hiện hữu."""
    if client.collection_exists(collection_name):
...
def _get_status_attr(status_obj, key: str, default=None):
    """Đọc thuộc tính trạng thái theo cả hai kiểu object hoặc dict để tránh phụ thuộc implementation."""
...
def _normalize_doc_status_value(raw_status) -> str:
    """Chuẩn hóa giá trị status về chuỗi chữ thường để dễ thống kê và so sánh."""
